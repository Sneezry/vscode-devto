...
export class API {
  ...
  private async _list(page: number) {
    const options = this._buildRequestOptions('/articles/me', 'GET', {page});
  ...
  async update(id: number, title: string, bodyMarkdown: string) {
    const options = this._buildRequestOptions('/articles/' + id, 'PUT', {title, body_markdown: bodyMarkdown});
    const response: Article = await rq(options);
    return response;
  }

  async create(title: string, bodyMarkdown: string) {
    const options = this._buildRequestOptions('/articles', 'POST', {title, body_markdown: bodyMarkdown});
    const response: Article = await rq(options);
    return response;
  }
}
